from flask import Flask, render_template, request, redirect, url_for, make_response, Response, session
...
plt.rcParams['lines.markersize'] = 7 # Change the radius of the points
...
app.config['SESSION_TYPE'] = 'filesystem' # https://stackoverflow.com/questions/43304363/simple-server-side-flask-session-variable
Session(app)
...
def ERROR(error_code:int, **kwargs) -> tuple:
    return {
        'status':        'error',
...
class ApiGenerateDataset(Resource):
    def func(self):
        ...
        if args['factor'] is not None and not ( 0 <= float(args['factor']) < 1 ):    return ERROR(8)
...
def node_plot():
    ...
    # Check if markersize was provided and if it is valid
    if args['markersize'] is None:                                                                return ERROR(13)
    if not ( LIMITS['markersize'][0] <= float(args['markersize']) <= LIMITS['markersize'][1] ):   return ERROR(12)
    ...
    # If the classifier is given, check if alpha was provided and if it is valid
    if args['classifier'] != 'none':
        if args['alpha'] is None:                                                       return ERROR(18)
        if not ( LIMITS['alpha'][0] <= float(args['alpha']) <= LIMITS['alpha'][1] ):    return ERROR(19)
    ...
    # Plot the data
    plt.rcParams['lines.markersize'] = float(args['markersize'])
    ...
    if args['classifier'] != 'none':
        ...
        DecisionBoundaryDisplay.from_estimator(clf, attrs_data, ax=ax, alpha=float(args['alpha']), cmap=args['cmap'])
